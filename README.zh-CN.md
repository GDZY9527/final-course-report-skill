...
本仓库使用自定义 **非商业许可证**。允许个人学习、教育、研究、评估和其他非商业目的下使用、复制、修改和分享。

未经版权所有者事先书面许可，禁止将本仓库或其修改版本用于商业销售、付费分发、转售、付费报告生成服务、付费培训产品、商业智能体包、SaaS 服务或其他以盈利为主要目的的活动。

分发副本或实质性部分时，必须保留版权声明、许可证文本，并注明原作者或原仓库来源。
...
项目方向：{project_direction}  # 或：暂无明确项目目标，请智能体自行设计唯一性项目。
模板：{template_docx}
...
4. 向智能体提供最小必需输入：
   - 课程名称
   - 项目方向；如果没有明确目标，应明确写“暂无明确项目目标，请智能体自行设计唯一性项目”
   - Word 模板路径
   - conda Python 路径
5. 可选提供覆盖项：
   - 目标课程目录：如果不希望智能体根据课程名称自动创建目录
   - 学生和封面字段：如果希望现在填入；否则封面字段留空，最后由用户在 Word 中自行填写
6. 只有当项目方向中明确表示“暂无明确项目目标”或“请智能体自行设计”时，智能体才应根据课程核心知识自行设计一个新的、未复用的、可交付项目。
...
该脚本会检查 DOCX 正文密度、嵌入图片数量、标题样式、目录是否沿用模板、前端截图、英文术语括号堆砌和 PNG 打印风险等问题。报告过短、图片未嵌入、旧目录残留、前端截图缺失或存在暗色图片时，应先修复再交付。
