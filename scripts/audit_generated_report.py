...
def main() -> int:
    ...
    docx = [inspect_docx(p, root) for p in root.rglob("*.docx") if not is_template_docx(p)]
    ...
    temp_files = [
        ...
        for p in root.rglob("*")
        if p.is_file() and (p.name.startswith("~$") or p.suffix.lower() in {".pyc", ".pyo"})
    ]
    if temp_files:
        failures.append(f"temporary/cache files should not enter delivery: {temp_files[:10]}")

    report = {"docx": docx, "png": png, "text": text_audit, "frontend": frontend, "temp_files": temp_files, "failures": failures}
